...
- **结论**：当前无需紧急优化，GPU 侧存在 2 个明确的低成本优化方向（Lumen 降级、后处理瘦身）
...
纹理内存池和流送纹理池已占满为正常现象，引擎会按需分配和释放内存，当前数值未超出正常范围。PixelShader Memory 208MB 对于 UE5 项目处于合理水平。
...
GPU 为当前瓶颈线程（12.99ms），但帧率稳定达标，暂无紧急优化必要。Game 线程余量充裕（7.8ms），后续若增加粒子、AI 等逻辑负载，Game 线程仍有足够余量承载。若未来需要释放 GPU 预算，应优先关注 Lumen 与后处理管线。


## 4. 后续方向

以下方向基于本次性能数据的推演，作为后续迭代的可选项，当前版本暂不执行。
...
如果后续要释放 GPU 空间，可以考虑：
- 将 Lumen 品质从 High 降至 Medium（预期收益 2-3ms）
- 或完全关闭 Lumen，改用烘焙静态光照（预期收益 4-6ms）

> 当前版本不做改动。Lumen 是否保留取决于后续关卡设计方向——若扩展为动态时间或可破坏场景，Lumen 的价值会相应提升，届时再根据实际需求评估取舍。

### 4.2 后处理管线

Postprocessing 占 GPU 时间约 30%，主要消耗来自 Motion Blur、Depth of Field 等效果。

在 Boss 战高速移动和频繁镜头切换的场景中，这些效果的实际感知较弱，且对操作判断没有帮助。建议优先级：
- 移除 Motion Blur（收益约 1ms，几乎无视觉损失）
- 移除 Depth of Field（收益约 0.5-1ms，战斗场景中很难注意到）
- Bloom 和 Tone Mapping 保留（对视觉风格影响较大）

> 当前版本不做改动。若后续 GPU 预算紧张，可优先移除 Motion Blur。

### 4.3 纹理流送

纹理池和流送池均已占满，当前项目资源量下未出现卡顿，但如果后续增加更多 Boss 或场景资源，可能会触发动态换页开销。

当前主要纹理的 Max Size 已在合理范围内，暂无调整必要。若后续资源量增加，可考虑：
- 根据实际显存调整 `r.Streaming.PoolSize`
- 检查新增纹理的导入设置，避免 4K 纹理被误导入

### 4.4 Shipping 版本复测

本次测试在 Development PIE 环境下进行，调试符号和额外日志会对性能产生一定影响。建议后续打包 Shipping 版本，关闭 VSync，固定 1080p / 100% Render Scale 后复测，获取更接近真实运行环境的性能基线。
